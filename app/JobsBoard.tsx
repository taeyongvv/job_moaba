...
import { TIERS, formatDeadline, type Job, type Tier } from "@/lib/jobradar";
...
export default function JobsBoard({ jobs }: { jobs: Job[] }) {
  ...
  const shown = useMemo(() => {
    ...
    return jobs.filter((j) => {
      if (activeCo !== "all" && j.company_key !== activeCo) return false;
  ...
  }, [jobs, activeCo, activeTier, query]);
          ...
          <div className={styles.chiprow}>
            ...
            {companies.map((c) => (
              <button
                ...
                type="button"
                className={styles.chip}
                aria-pressed={activeCo === c.key}
                onClick={() => setActiveCo(c.key)}
              >
